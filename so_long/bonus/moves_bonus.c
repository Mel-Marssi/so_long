...
/*   Updated: 2023/01/30 18:51:19 by mel-mars         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	norm_moves(t_long *p, t_pos x, char *str, int *move)
{
	...
	(*move)++;
	ptr = ft_itoa(*move);
	p->img = mlx_xpm_file_to_image(p->mlx, "textures/01.xpm", &p->hgt, &p->wth);
	ft_protect(p->img);
	mlx_put_image_to_window(p->mlx, p->win, p->img, 60, 0);
	mlx_string_put(p->mlx, p->win, 60, 0, 0x00FFFF, ptr);
	free(ptr);
}
